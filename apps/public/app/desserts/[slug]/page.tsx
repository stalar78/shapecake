...
export default async function DessertPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <PublicHeader />
      <main>
        {dessert.is_available ? <script type="application/ld+json" dangerouslySetInnerHTML={jsonLd(dessertJsonLd(dessert))} /> : null}
        <section className="public-shell grid gap-12 py-12 md:py-20 lg:grid-cols-[1.08fr_0.92fr]">
          <div className="grid gap-4">
            <Link className="quiet-link w-fit" href="/">Назад к каталогу</Link>
          ...
          <article className="self-start pt-8">
            ...
            <p className="mt-7 text-lg leading-8 text-[var(--muted)]">{dessert.full_description || dessert.short_description || "Описание скоро появится."}</p>
              ...
              <div className="mt-5 grid gap-1">
                {dessert.variants.map((variant) => (
                  <div className="grid grid-cols-[1fr_auto] gap-4 border-b border-[var(--line)] py-4" key={variant.id}>
                    <p className="font-bold">{variant.weight_value} {variant.weight_unit}</p>
            ...
            <dl className="mt-12 grid gap-6 border-t border-[var(--line)] pt-7 text-sm text-[var(--muted)]">
              <div><dt className="font-bold text-[var(--foreground)]">Состав</dt><dd className="mt-2 whitespace-pre-wrap">{dessert.ingredients || "Не указан"}</dd></div>
              <div><dt className="font-bold text-[var(--foreground)]">Аллергены</dt><dd className="mt-2 whitespace-pre-wrap">{dessert.allergens || "Не указаны"}</dd></div>
              {dessert.warnings ? <div><dt className="font-bold text-[var(--foreground)]">Важно</dt><dd className="mt-2 whitespace-pre-wrap">{dessert.warnings}</dd></div> : null}
            </dl>
          </article>
        </section>
